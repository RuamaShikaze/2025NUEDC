...
uart = UART(3, 9600)  # 例如使用UART 3，波特率9600
...
# 初始化PID控制器
laser = Laser("P2")
pan_pid = PID(p=0.03, i=0.012, d = 0.005,imax=90)
tilt_pid = PID(p=0.008, i=0.015, d=0.005, imax=90)
...
BLACK_THRESHOLD = (0, 50, -20, 20, 20, -20)  # 黑色阈值
...
while True:
    ...
    clock.tick()
    img = sensor.snapshot()
    #img.adaptive_threshold(5, 5, offset=10)
    laser.off()
    red_led.off()
            ...
            if single_black_blob.pixels() / (single_black_blob.w() * single_black_blob.h()) < 0.3:
                ...
                if white_blobs:  # 如果有目标
                    ...
                    if (2 < largest_white.pixels() / single_black_blob.pixels() < 4) and \
                        ...
                        pan_error = raw_x - IMAGE_CENTER_X
                        tilt_error = raw_y - IMAGE_CENTER_Y
                        if (pan_error-10<=2.5) and (tilt_error-1<=2.5) and (pan_error-10>= -2.5) and (tilt_error-1>= -2.5):
                            laser.on()
                            print("Laser on")
                        ...
                        #data = struct.pack('<ffff', pan_error,tilt_error,pan_output,tilt_output)
