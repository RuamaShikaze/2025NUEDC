...
pan_servo = Servo(1)
tilt_servo = Servo(2)
pan_servo.calibration(500, 2500, 2500)  # 校准舵机范围
tilt_servo.calibration(500, 2500, 1500)

button1 = Pin('P3', Pin.IN, Pin.PULL_UP)  # 按钮1，上拉输入
button2 = Pin('P4', Pin.IN, Pin.PULL_UP)  # 按钮2，上拉输入
...
# PID控制器参数（保持原始参数）
pan_pid = PID(p=0.008, i=0.015, d=0.006, imax=90)
...
while True:
                    ...
                    if (2 < largest_white.pixels() / black_blob.pixels() < 4) and \
                       ...
                       abs(largest_white.cy() - black_blob.cy()) < 10:
                        ...
                        print(f"err_x=%.1f,err_y=%.1f,out_pan=%.1f,out_tilt=%.1f" %
                              (pan_error, tilt_error, pan_output, tilt_output),is_scanning)
    ...
    # 扫描模式逻辑（修正版）
    if is_scanning:
        turn_angle = SCAN_SPEED * dt * scan_direction
        new_pan_angle = pan_servo.angle() + turn_angle  # 方向与角度变化一致

        # 边界检测：限制在 -180° 到 180° 之间
        if new_pan_angle >= 180:
            new_pan_angle = 180
            scan_direction = -1  # 触达右边界，反转方向（顺时针）
        elif new_pan_angle <= -180:
            new_pan_angle = -180
            scan_direction = 1   # 触达左边界，反转方向（逆时针）
