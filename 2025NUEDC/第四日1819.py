...
button1 = Pin('P6', Pin.IN, Pin.PULL_UP)  # 是否开始
...
# PID控制器参数（保持原始参数）
pan_pid = PID(p=0.018, i=0.035, d=0.008, imax=90)
tilt_pid = PID(p=0.066, i=0.03, d=0.006, imax=90)
...
while True:
                ...
                if white_blobs:
                    ...
                    if (2 < largest_white.pixels() / black_blob.pixels() < 4) and \
                       ...
                       abs(largest_white.cy() - black_blob.cy()) < 10:
                        ...
                        # 激光瞄准逻辑（保持原始）
                        if (pan_error <= 6) and (pan_error >= -6) and (tilt_error <= 6) and (tilt_error >= -6):
                            if button1.value() == 1:
                                laser.on()
                        ...
                        tilt_output = clamp(tilt_output, -5, 5)
                        if button1.value() == 0:
                            pan_output = 0
                            new_pan = 0
                            tilt_output = 0
    ...
    if is_scanning:
        # 计算扫描角度(连续转动)
        turn_angle = SCAN_SPEED * dt * scan_direction * 2
